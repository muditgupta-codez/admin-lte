...
const StatusModal = ({ data, handleClose, show, loadData }) => {
  ...
  const arr = ["Assigned", "Out For Delivery", "Delivered"];
    ...
    <Modal show={show} onHide={handleClose}>
      ...
      <Modal.Body>
        <div className="form-group">
          ...
          >
            {arr.map((item, i) => (
              <option id={item} key={i} value={i + 1}>
                {item}
              </option>
            ))}
          </select>
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (c) => {
        ...
        return (
          <div>
            <span>First Name: {details.first_name}</span>
            <br />
            <span>Last Name: {details.last_name}</span>
            <br />
            {details?.company ? (
              <>
            ...
            <span>Address 1: {details.address_1}</span>
            <br />
            {details?.address_2 ? (
              <>
                <span>Address 1: {details.address_1}</span>
            ...
            <span>
              City: {details.city} ({details.postcode})
            </span>
            <br />
            <span>Country: {details.country}</span>
            <br />
            <span>State: {details.state}</span>
            <br />
          </div>
        );
      },
    ...
    },
    {
      ...
      formatter: (c) => {
        return (
          <mark class={"order-status status-" + c}>
            {c === 1 ? <span>Assigned</span> : ""}
            {c === 2 ? <span>Out For Delivery</span> : ""}
            {c === 3 ? <span>Delivered</span> : ""}
          </mark>
        );
      },
    },
